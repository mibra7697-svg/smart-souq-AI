    ...
    constructor() {
        ...
        this.APIS = {
            ...
            noon: {
                ...
                apiKey: process.env.REACT_APP_NOON_API_KEY
            },
            localStores: [
...
export const searchWithRealData = async (query, category = 'all', options = {}) => {
    ...
    // استخدام البيانات الحقيقية إذا كانت APIs مفعلة
    if (process.env.REACT_APP_USE_REAL_DATA === 'true') {
        const results = await service.searchRealProducts(query, category);
        return service.createSuccessResponse(results, query);
    }
    
    // العودة للبيانات المحلية (للتنمية أو إذا تعطلت APIs)
